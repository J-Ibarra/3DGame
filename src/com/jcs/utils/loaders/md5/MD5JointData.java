...
public class MD5JointData {
    ...
    public MD5JointData(String name, int parentIndex, Vector3f position, Quaternionf orientation) {
        this.name = name;
    ...
    public static MD5JointData processLine(String line) {
        String name = null;
        int parentIndex = -2;
        ...
        if (matcher.matches()) {
            ...
            orientation = calculateQuaternion(new Quaternionf(x, y, z, 0));
        }

        if (name == null || parentIndex == -2 || position == null || orientation == null)
            throw new RuntimeException();
        return new MD5JointData(name, parentIndex, position, orientation);
    }
